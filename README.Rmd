...
The goal of `{ec}` is to provide a lightweight encapsulation system for **R** object-oriented programming.

## Installation

You can install the development version of `{ec}` from [GitHub](https://github.com/) with:
...
`{ec}` offers another avenue for quick, flexible class setups, primarily for advanced users.
...
Firstly, because I got too curious about playing around with **R**'s environments and bindings.
Secondly, out of some inspiration from **python**'s `Class` implementation, and how properties and methods are simply defined within the class body.
In **R**'s case, we can pass an expression to specific environment and use a functions instead of decorators to define **properties** with _getters_ and _setters_.
...
The `{ec}` classes also provide a similar experience when creating a new class.
Like `ReferenceClasses`, we can simply call the closer object `Person()` to create a new instance of the class.

...
# we don't want this to be in ls(global)
...
contain({
  ...
  on.exit({
    ...
    local(
      for (binding in names(.__capsule__.)) {
  ...
  .__create__. <- function() {
    capsule <- new.env(parent = global)
    list2env(
      lapply(
        as.list.environment(global, all.names = TRUE),
        function(x) {
          if (is.function(x)) {
